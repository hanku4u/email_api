...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db.commit() # Commit the session to save the new user to the database
    
    db.refresh(db_user) # Refresh the user object to get the ID that was generated by the database
...
def delete_user(db: Session, user_id: int):
    ...
    db.commit() # Commit the session to delete the user from the database
...
def subscribe_user_to_email_types(db: Session, userID: int, email_type_ids: List[int]):
    ...
    for email_id in email_type_ids:
        ...
        db.add(user_email_type) # Add the UserEmailType object to the database

        db.commit() # Commit the changes to the database

    db.refresh(user)    # Refresh the user object to get the latest version from the database
